...
# 获取脚本目录
SCRIPT_DIR = os.path.dirname(os.path.abspath('train_final_optimized.py'))
...
try:
    ...
    print('✓ utils.optimized_violation_logger 导入成功')
except ImportError as e:
    print(f'✗ utils.optimized_violation_logger 导入失败: {e}')

try:
    from utils.reward_tracker import RewardTracker
    print('✓ utils.reward_tracker 导入成功')
except ImportError as e:
    print(f'✗ utils.reward_tracker 导入失败: {e}')

try:
    from utils.debug_logger import DebugLogger, ErrorRecovery
    print('✓ utils.debug_logger 导入成功')
except ImportError as e:
    print(f'✗ utils.debug_logger 导入失败: {e}')
...
try:
    from train_final_optimized import get_petri_net
...
echo "运行命令:"
echo "python scripts/train_final_optimized.py --train $CONSTRAINED_FLAG --m-success $SUCCESS --m-cars-driven $CARS_DRIVEN --m-waiting-time $WAITING_TIME --m-max-waiting-time $MAX_WAITING_TIME --m-timestep $TIMESTEP"
echo ""

# 运行训练
python scripts/train_final_optimized.py --train $CONSTRAINED_FLAG \
  --m-success $SUCCESS \
